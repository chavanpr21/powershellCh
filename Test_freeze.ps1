...
try{
	if ($RdPreDeploy.Checked -eq "true"){
        Write-Host "PreDeployment Checks have Started"
        #$UI_FolderStructure = Start-Job -FilePath $scriptfolder -ArgumentList $argumentfile -Name "UI_FolderStructure"
      $UI_FolderStructure = Start-Process -FilePath $scriptfolder -ArgumentList $argumentfile
        Do {[System.Windows.Forms.Application]::DoEvents()} Until ($UI_FolderStructure.State -eq "Completed")
		<#
